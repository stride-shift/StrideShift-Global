...
interface GenerativeMountainSceneProps {
  /** Mesh tint, e.g. '#7dd3fc' for sky-blue or '#1e3a5f' for a darker night version. */
  color?: string;
  /** Animation speed multiplier (default 1). 0 to pause. */
  speed?: number;
}

/**
 * Solid, undulating mountain landscape rendered with a custom Perlin-noise
 * displacement shader. Cursor-reactive point light for soft highlights. The
 * canvas fills its parent (which must be `position: relative`).
 */
export function GenerativeMountainScene({
  ...
  useEffect(() => {
    ...
    const animate = (t: number) => {
      material.uniforms.time.value = (t * 0.0003) * speed;
